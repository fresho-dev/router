...
function printResult(r) {
  console.log(`${r.name.padEnd(30)} ${(r.size/1024).toFixed(1).padStart(6)} KB  (${(r.gzip/1024).toFixed(1)} KB gzip)`);
}
...
// Test cases
const coreOnly = await measure('Core only', `
...
const withValidation = await measure('With validation', `
  import { route, router } from './src/index.ts';
  const api = router('/api', {
    hello: route({
      method: 'get',
...
const withHttpClient = await measure('With httpClient', `
  import { route, router } from './src/index.ts';
  const api = router('/api', {
    hello: route({ method: 'get', path: '/hello', handler: () => ({ msg: 'hi' }) }),
  });
...
const withLocalClient = await measure('With localClient', `
  import { route, router } from './src/index.ts';
  const api = router('/api', {
    hello: route({ method: 'get', path: '/hello', handler: () => ({ msg: 'hi' }) }),
  });
...
const withBothClients = await measure('With both clients', `
  import { route, router } from './src/index.ts';
  const api = router('/api', {
    hello: route({ method: 'get', path: '/hello', handler: () => ({ msg: 'hi' }) }),
  });
...
const handlerOnly = await measure('Handler only (no clients)', `
  import { route, router } from './src/index.ts';
  const api = router('/api', {
    hello: route({ method: 'get', path: '/hello', handler: () => ({ msg: 'hi' }) }),
  });
...
const withMiddleware = await measure('+ cors, errorHandler', `
  import { route, router } from './src/index.ts';
...
const withDocs = await measure('With generateDocs', `
...
const withAllMiddleware = await measure('+ all middleware', `
  import { route, router } from './src/index.ts';
